...
class ProSubscriptionViewModel: ObservableObject {
    ...
    func purchaseProduct() {
        ...
        alertView.addAction(title: "Imt.newapp.iap.buy".i18nImt(), style: .cancel) { [weak self] _ in
    ...
    func trackPurchaseEvent(info: ProSubscriptionInfo, imtSessionId: Int) {
        ...
        AdjustTrackManager.shared.event("lk8rlx", revenue: userInfo?.iosPlanTier == "trial" ? (0.0, currencyCode) : (amount, currencyCode), extraParams: ["imtSessionId": imtSessionId], callbackParams: ["user_id": "\(userInfo?.uid ?? 0)"])
